...
if [[ -d $sample/output/Gene/filtered/ && -s $sample/Log.out ]]
then 
  PAIRED="Single"
...
elif [[ -d $sample && -d $sample/output && -s $sample/Log.out ]]
then 
## no filtered output - this is usually Smart-seq2 
  PAIRED="Single"
  if [[ `grep "mate 2" $sample/Log.out` != "" ]]
  then
    PAIRED="Paired"
  fi
  ...
  WL="Undef"
  if [[ `grep "manifest" $sample/Log.out` != "" ]]
  then
  WL="None(Smart-seq)"
  fi
  ...
  F1=`grep "Reads Mapped to GeneFull: Unique+Multip.*e GeneFull," $sample/output/GeneFull/Summary.csv | awk -F "," '{print $2}'`
  F2=`grep "Reads Mapped to GeneFull: Unique GeneFull," $sample/output/GeneFull/Summary.csv | awk -F "," '{print $2}'`
  C=`zcat $sample/output/Gene/raw/barcodes.tsv.gz | wc -l`
  R2=`grep "yessubWLmatch_UniqueFeature" $sample/output/GeneFull/Features.stats | awk '{print $2}'`
  CF=`echo $R1 | awk -v v=$R2 '{printf "%.3f\n",v/$1}'`
  R3=`grep "yesUMIs" $sample/output/GeneFull/Features.stats | awk '{print $2}'`
    ## look ma, no hands!
    GC=`zcat $sample/output/GeneFull/raw/matrix.mtx.gz | awk 'NR>3' | awk '{print $2}' | uniq -c | awk '{print $1}' | sort -n | awk '{a[i++]=$1} END {print a[int(i/2)]}'`
  ST=`grep "^soloStrand" $sample/Log.out | grep RE-DEFINED | awk '{print $2}'`
